import logging
from ..exc import NoSuchApplication
from .expression import ExpressionAbs, ExpressionField
...
class Condition(object):
    ...
    def existAction(self, act, val, inline=False):
        ...
        for a in self.actions:
            if a.get('application') == act and a.get('data') == val and a.get(
                    'inline') == inline:
                return True
        return False
...
class or_(object):
    '''
        Class to add conditions to an extensions to be evaluated with logical \
            OR. 
