...
class User(object):
    ...
    def __init__(self, user_id, cacheable=None, password=None):
    ...
    def addParameter(self, param, val):
        ...
        log.warning('Cannot replace existing parameter. (%s) %s for %s', param,
                    self.getParameter(param), val)
        raise ValueError
    ...
    def todict(self):
        ...
        if self.cacheable:
            attrs['cacheable'] = self.cacheable
