from cloud.fs.models import service_get_gateway, service_get_mobile
...
class Dialplan(BaseXml):
    ...
    def get_xml_data(self):
        ...
        if direction == 'inbound':
            gateway = service_get_gateway(context)  # 获取企业配置网关
            if not gateway:
                gateway = fs_settings.DEFAULT_GATEWAY_NAME
            mobile = service_get_mobile(phoneId)  # 获取真实被叫
            if not mobile:
                return self.hangup()
            return [
                ('sys_inbound', False,
                 [('destination_number', '^(.*)$', False, [
                     ...
                     ('set', 'effective_caller_id_number={0}'.format(caller),
                      False),
                     ('bridge',
                      'sofia/gateway/{0}/{1}'.format(gateway, mobile), False),
                 ])]),
            ]
    ...
    def hangup(self):
        return [
            ('hangup', False, [('destination_number', '^(.*)$', False, [
                ('hangup', None, False),
            ])]),
        ]
